...
//Run the Ajax Code
    function do_ajax(action, request, init_count) {
        $.ajax({
            url: '../controller/page_request_cs.php?request=' + request + '&action=' + action+'&source=CREDITSCORES',
    ...
    $(".filter").on('click', function (event) {
        ...
        table.ajax.url('../controller/page_request_cs.php?request=' + param.request + '&action=' + param.action + '&filterID=' + param.filterid+'&source=CREDITSCORES').load();
    ...
    function format(d) {
        ...
        if (d.contract_folderpath != null) {
            var agreementpath = d.contract_folderpath.replace("/var/www/html", "http://localhost");
        ...
        return '<div style="padding:10px;width:95%;background-color:#ffffff">' +
                ...
                '<tr><td>' + d.street_address + '</td><td> Income:' + d.income + '&nbsp;&nbsp;&nbsp;&nbsp;' + 'Setup Fee: ' + d.setup_fee + '</td><td rowspan=3>' + link1 + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' + link2 + ' &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <br /><br />' + link3 +
                '</td><td>' + button1 + '</td></tr>' +
    ...
    $(document).on('click', '.redoagreement', function () {
        ...
        $.ajax({
            url: '../controller/page_request_cs.php?request=' + request + '&action=' + action + "&clientID=" + client_id+'&source=CREDITSCORES',
            success: function (result) {
                //console.log(result);
                var file = result.split("|");
                var filepath = file[0].replace("/var/www/html", "http://localhost");
                var content = '<font style="color:red">Redo Agreement has been successful!! </font><br /><br /><a target="_blank" href="' + filepath + file[1] + '" class="btn btn-warning btn-sm" >Agreement 1</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a target="_blank" href="' + filepath + file[2] + '" class="btn btn-warning btn-sm" >Agreement 2</a>';
    ...
    $(document).on('click', '.saveredoagreement', function () {
        ...
        $.ajax({
            url: '../controller/page_request_cs.php?request=' + request + '&action=' + action + "&clientID=" + client_id + "&redoFilePath=" + filepath+'&source=CREDITSCORES',
            success: function (result) {
    ...
    $(document).on('click', '.sendAppemail', function () {
        var client_id = $(this).attr('id');
        window.location.href = "send_email_cs.php?ClinetID=" + client_id;
    });
    //Click event for redo Button
