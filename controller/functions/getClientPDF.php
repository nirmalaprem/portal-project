...
if($_POST){
  ...
  if ($selected_mattress != '') {
	$agreement_path3="http://checkout.creditcanada.net".$contracts_arrayurl[2];
  }
  ...
  if(mkdir($contract_path)) { // Make directory in deal portal back end
	...
	if ($selected_mattress != '') {
		$PDF_CONTENTS = file_get_contents($agreement_path3); //cc agreement
		file_put_contents($contract_path."/".$last_name."_".$first_name."_mattress_finance.pdf", $PDF_CONTENTS);
	}
